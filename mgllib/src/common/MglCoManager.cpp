...
void CMglCoManager::InitSprite()
{
	if ( m_pD3dDev == NULL )
		MyuThrow( MGLMSGNO_GRPMGR(80), "CMglCoManager::InitSprite()  m_pD3dDevがNULL" );
	if ( m_pSprite != NULL )	//	既にあればスキップ
		return;

	MyuAssert2( D3DXCreateSprite( m_pD3dDev, &m_pSprite ), D3D_OK,
		MGLMSGNO_GRPMGR(81), "CMglCoManager::Init()  D3DXCreateSprite()に失敗" );
}

//	スプライトのBegin()
void CMglCoManager::Begin(DWORD dwFlag)
{
	if( m_pSprite == NULL )
...
#endif
			MGLMSGNO_GRPMGR(89), "CMglCoManager::Begin()  m_pSprite->Begin()に失敗" );
		m_bSpriteBegun = TRUE;
	}
}

//	スプライトのEnd()
void CMglCoManager::End()
{
	if( m_pSprite == NULL )
	...
	if( m_pSprite != NULL && m_bSpriteBegun == TRUE ){
		MyuAssert2( m_pSprite->End(), D3D_OK,
			MGLMSGNO_GRPMGR(90), "CMglCoManager::End()  m_pSprite->End()に失敗" );
		m_bSpriteBegun = FALSE;
	}
}

//	スプライト描画
void CMglCoManager::Draw( _TEX *pTexture, float x, float y, CONST RECT* pSrcRect, D3DCOLOR color,
		float fScaleX, float fScaleY, float fRotationCenterX, float fRotationCenterY, float fAngle )
{
	//	チェック
	if ( m_pSprite == NULL )
		MyuThrow( MGLMSGNO_GRPMGR(83), "CMglCoManager::Draw()  スプライトが無効になっているか、スプライトの初期化に失敗しました。" );
	if ( pTexture->GetDirect3dTexturePtr() == NULL )
		MyuThrow( MGLMSGNO_GRPMGR(84), "CMglCoManager::Draw()  pTexture がNULLです。テクスチャが初期化されていない可能性があります。" );
...
*/
	...
	MyuAssert2( m_pSprite->SetTransform(&matrix), D3D_OK,
		MGLMSGNO_GRPMGR(85), "CMglCoManager::Draw()  m_pSprite->SetTransform()に失敗" );
...
//					 &D3DXVECTOR3(0.5f, 0.5f, 0),
		...
		MGLMSGNO_GRPMGR(86), "CMglCoManager::Draw()  m_pSprite->Draw()に失敗" );
...
//		MGLMSGNO_GRPMGR(86), "CMglCoManager::Draw()  m_pSprite->Draw()に失敗" );
	...
	MyuAssert2( m_pSprite->Draw( pTexture->GetDirect3dTexturePtr(),
		...
		MGLMSGNO_GRPMGR(87), "CMglCoManager::Draw()  m_pSprite->Draw()に失敗" );
#endif
}
