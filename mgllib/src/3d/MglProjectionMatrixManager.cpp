...
void CMglProjectionMatrixManager::SetCamera(D3DXMATRIX &matTarget, D3DXMATRIX& matPos, float fRotate)
{
...
void CMglProjectionMatrixManager::SetCamera(D3DXVECTOR3 &vecTarget, D3DXVECTOR3 &vecPos, float fRotate)
{
...
void CMglProjectionMatrixManager::SetCamera(
...
//	カメラ位置の設定
//void CMglProjectionMatrixManager::SetCameraPos(float x, float y, float z)
void CMglProjectionMatrixManager::CameraLockAt(float fPosX, float fPosY, float fPosZ, float fTargetX, float fTargetY, float fTargetZ, float fRotate)
{
	InitCheck();
#if _MGL3D_COORDINATE_USE == _MGL3D_COORDINATE_LEFT_HAND
	D3DXMatrixLookAtLH(
	...
	MyuAssert( m_pD3dDev->SetTransform(D3DTS_VIEW, &m_matView), D3D_OK,
		"CMglProjectionMatrixManager::SetCamera()  SetTransform()に失敗" );
}
...
//	カメラの注目点を変更
void CMglProjectionMatrixManager::SetCameraViewTarget(float x, float y, float z)
{
	SetCamera(m_fCameraPosX, m_fCameraPosY, m_fCameraPosZ, x, y, z);
}
//	カメラを移動（シフト）
void CMglProjectionMatrixManager::MoveCamera(float x, float y, float z)
{
	SetCamera(
...
//	カメラをX軸方向に回転
void CMglProjectionMatrixManager::CameraRotation(int direction, float fAngle)
{
	InitCheck();
...
void CMglProjectionMatrixManager::SetCameraAngle(float fAngleX, float fAngleY, float fAngleZ)
{
	InitCheck();
...
void CMglProjectionMatrixManager::Rotate(float fAngleX, float fAngleY, float fAngleZ)
{
	m_fRotateX += fAngleX;
	m_fRotateY += fAngleY;
	m_fRotateZ += fAngleZ;
...
void CMglProjectionMatrixManager::ReTransform()
{
	...
	//	View（カメラ）
	MyuAssert( m_pD3dDev->SetTransform(D3DTS_VIEW, &m_matView), D3D_OK,
		"CMglProjectionMatrixManager::ReTransform()  SetTransform(D3DTS_VIEW)に失敗" );
...
//	View設定
void CMglProjectionMatrixManager::SetViewMatrix(D3DXMATRIX &matView)
{
	InitCheck();
	m_matView = matView;
	MyuAssert( m_pD3dDev->SetTransform(D3DTS_VIEW, &m_matView), D3D_OK,
...
/*	↓移動するだけで全然駄目・・・
//	カメラをX軸方向に回転
void CMglProjectionMatrixManager::SetCameraAngle2(float fAngleX, float fAngleY, float fAngleZ)
{
	D3DXMATRIX matX;
	D3DXMATRIX matY;
	D3DXMATRIX matZ;
