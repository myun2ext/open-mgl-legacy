...
void CMglMatrixManager::Init( CMglGraphicManager* in_myudg )
{
	...
	MyuAssert( m_pD3dDev->SetTransform(D3DTS_WORLD, &m_matWorld), D3D_OK,
		"CMglMatrixManager::Init()  SetTransform(D3DTS_WORLD)に失敗" );
	*/
...
//	Projectionの設定
void CMglMatrixManager::SetupProjection( float fAspectRatio, float fViewingAngle, float fClipNear, float fClipFar )
{
	InitCheck();
	...
	MyuAssert( m_pD3dDev->SetTransform(D3DTS_PROJECTION, &m_projection), D3D_OK,
		"CMglMatrixManager::SetupProjection()  SetTransform()に失敗" );
...
void CMglMatrixManager::SetCamera(float fPosX, float fPosY, float fPosZ, float fTargetX, float fTargetY, float fTargetZ)
{
	InitCheck();
...
//	カメラ位置の設定
//void CMglMatrixManager::SetCameraPos(float x, float y, float z)
void CMglMatrixManager::CameraLockAt(float fPosX, float fPosY, float fPosZ, float fTargetX, float fTargetY, float fTargetZ)
{
	InitCheck();
#if _MGL3D_COORDINATE_USE == _MGL3D_COORDINATE_LEFT_HAND
	D3DXMatrixLookAtLH(
	...
	MyuAssert( m_pD3dDev->SetTransform(D3DTS_VIEW, &m_matView), D3D_OK,
		"CMglMatrixManager::SetCamera()  SetTransform()に失敗" );
}
...
//	カメラの注目点を変更
void CMglMatrixManager::SetCameraViewTarget(float x, float y, float z)
{
	SetCamera(m_fCameraPosX, m_fCameraPosY, m_fCameraPosZ, x, y, z);
}
//	カメラを移動（シフト）
void CMglMatrixManager::MoveCamera(float x, float y, float z)
{
	SetCamera(
...
//	カメラをX軸方向に回転
void CMglMatrixManager::CameraRotation(int direction, float fAngle)
{
	InitCheck();
...
void CMglMatrixManager::SetCameraAngle(float fAngleX, float fAngleY, float fAngleZ)
{
	InitCheck();
...
//	ワールドを設定
void CMglMatrixManager::SetWorld(
...
void CMglMatrixManager::Rotate(float fAngleX, float fAngleY, float fAngleZ)
{
	m_fRotateX += fAngleX;
	m_fRotateY += fAngleY;
	m_fRotateZ += fAngleZ;
...
*/
	MyuAssert( m_pD3dDev->SetTransform(D3DTS_WORLD, &m_matWorld), D3D_OK,
		"CMglMatrixManager::ReTransform()  SetTransform(D3DTS_WORLD)に失敗" );

	//	View（カメラ）
	MyuAssert( m_pD3dDev->SetTransform(D3DTS_VIEW, &m_matView), D3D_OK,
		"CMglMatrixManager::ReTransform()  SetTransform(D3DTS_VIEW)に失敗" );
...
//	ワールド設定
void CMglMatrixManager::SetWorldMatrix(D3DXMATRIX &matWorld)
{
	InitCheck();
	m_matWorld = matWorld;
	MyuAssert( m_pD3dDev->SetTransform(D3DTS_WORLD, &m_matWorld), D3D_OK,
...
//	View設定
void CMglMatrixManager::SetViewMatrix(D3DXMATRIX &matView)
{
	InitCheck();
	m_matView = matView;
	MyuAssert( m_pD3dDev->SetTransform(D3DTS_VIEW, &m_matView), D3D_OK,
...
//	Projection設定
void CMglMatrixManager::SetProjectionMatrix(D3DXMATRIX &matProjection)
{
	InitCheck();
...
/*	↓移動するだけで全然駄目・・・
//	カメラをX軸方向に回転
void CMglMatrixManager::SetCameraAngle2(float fAngleX, float fAngleY, float fAngleZ)
{
	D3DXMATRIX matX;
	D3DXMATRIX matY;
	D3DXMATRIX matZ;
