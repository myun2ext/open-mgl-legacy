#include "stdafx.h"
#include "MglCustomMesh.h"

//	コンストラクタ
CMglCustomMesh::CMglCustomMesh()
{
	m_pMesh          = NULL;
...
//	Xファイル読み込み
void CMglCustomMesh::Load(const char* szMeshFilePath)
{
	InitCheck();
	CreatedCheck();
	...
	if ( !msl::IsExistFile(szMeshFilePath) )
		MyuThrow(MGLMSGNO_MESH(2), "CMglCustomMesh::Load()  メッシュファイル \"%s\" は見つかりません。", szMeshFilePath);
	...
	MyuAssert2( D3DXLoadMeshFromX( (char*)szMeshFilePath, D3DXMESH_SYSTEMMEM, 
								   ...
								   &m_pMesh ), D3D_OK,
		MGLMSGNO_MESH(4), "CMglCustomMesh::Load()  D3DXLoadMeshFromX()に失敗" );
...
//	描画
void CMglCustomMesh::Draw()
{
	InitCheck();
	if ( m_pMesh == NULL )
		MyuThrow(MGLMSGNO_MESH(3), "CMglCustomMesh::Draw()  メッシュが読み込み、または作成されていません。");

	for( DWORD i=0; i < m_dwMaterialCount; i++ )
	{
		if ( m_pMeshMaterials != NULL )
			MyuAssert2( m_pD3dDev->SetMaterial( &m_pMeshMaterials[i] ), D3D_OK,
				MGLMSGNO_MESH(129), "CMglCustomMesh::Draw()  m_pD3dDev->SetMaterial()に失敗" );

		if ( m_pMeshTextures != NULL && m_pMeshTextures[i] != NULL )
			MyuAssert2( m_pD3dDev->SetTexture( 0, m_pMeshTextures[i] ), D3D_OK,
				MGLMSGNO_MESH(130), "CMglCustomMesh::Draw()  m_pD3dDev->SetTexture()に失敗" );

		MyuAssert2( m_pMesh->DrawSubset( i ), D3D_OK,
			MGLMSGNO_MESH(128), "CMglCustomMesh::Draw()  m_pMesh->DrawSubset()に失敗" );
	}
}
...
//	多角形メッシュ作成
/*void CMglCustomMesh::CreatePolygon(float fSideSize, float fHeight, float fDepth,
		D3DXCOLOR color, D3DXCOLOR ambient, D3DXCOLOR specular, D3DXCOLOR emissive, float fSpecularPower)
{
	...
	MyuAssert2( D3DXCreatePolygon( m_d3d, fSideSize, fHeight, fDepth, &m_pMesh, NULL), D3D_OK,
		MGLMSGNO_MESH(32), "CMglCustomMesh::CreateBox()  D3DXCreateBox()に失敗" );
...
//	箱メッシュ作成
void CMglCustomMesh::CreateBox(float fWidth, float fHeight, float fDepth,
		D3DXCOLOR color, D3DXCOLOR ambient, D3DXCOLOR specular, D3DXCOLOR emissive, float fSpecularPower)
{
	InitCheck();
	CreatedCheck();
...
//	円柱メッシュ作成
void CMglCustomMesh::CreateCylinderEx(float fWidthNear, float fWidthFar, float fHeight,
		D3DXCOLOR color, D3DXCOLOR ambient, D3DXCOLOR specular, D3DXCOLOR emissive, float fSpecularPower,
		UINT nDetailCount, UINT nVerticalVertexCount)
{
	...
	MyuAssert2( D3DXCreateCylinder( m_d3d, fRadiusNear, fRadiusFar, fHeight,
		...
		MGLMSGNO_MESH(40), "CMglCustomMesh::CreateCylinderEx()  D3DXCreateCylinder()に失敗" );
...
//	Teapotメッシュ作成
void CMglCustomMesh::CreateTeapot(
		D3DXCOLOR color, D3DXCOLOR ambient, D3DXCOLOR specular, D3DXCOLOR emissive, float fSpecularPower)
{
	InitCheck();
	CreatedCheck();
	...
	MyuAssert2( D3DXCreateTeapot( m_d3d, &m_pMesh, NULL), D3D_OK,
		MGLMSGNO_MESH(72), "CMglCustomMesh::CreateTeapot()  D3DXCreateTeapot()に失敗" );

	CreateSingleMaterial(color,ambient,specular,emissive,fSpecularPower);
}

//	ドーナツメッシュ作成
void CMglCustomMesh::CreateDonutEx(float fSize, float fRingDiameter, UINT nCircleDetail, UINT nCylinderDetail,
		D3DXCOLOR color, D3DXCOLOR ambient, D3DXCOLOR specular, D3DXCOLOR emissive, float fSpecularPower)
{
	InitCheck();
	CreatedCheck();
	...
	MyuAssert2( D3DXCreateTorus( m_d3d, fInnerRadius, fOuterRadius,
		...
		MGLMSGNO_MESH(80), "CMglCustomMesh::CreateDonut()  D3DXCreateTorus()に失敗" );

	CreateSingleMaterial(color,ambient,specular,emissive,fSpecularPower);
}

//	球体メッシュ作成
void CMglCustomMesh::CreateSphereEx(float fSize, UINT nHorizontalDetail, UINT nVerticalDetail,
		D3DXCOLOR color, D3DXCOLOR ambient, D3DXCOLOR specular, D3DXCOLOR emissive, float fSpecularPower)
{
	InitCheck();
	CreatedCheck();
	...
	MyuAssert2( D3DXCreateSphere( m_d3d, fRadius,
		...
		MGLMSGNO_MESH(80), "CMglCustomMesh::CreateSphereEx()  D3DXCreateSphere()に失敗" );
...
//	ひとつーだけのマテリアルー（謎
void CMglCustomMesh::CreateSingleMaterial(
	D3DXCOLOR color, D3DXCOLOR ambient, D3DXCOLOR specular, D3DXCOLOR emissive, float fSpecularPower)
{
	//m_dwMaterialCount = 1;
	CreateMaterials(1);
