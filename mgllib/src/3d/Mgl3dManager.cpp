...
int g_nSfcCount=0;
int g_2dgCount=0;
CMgl3DManager *g_pDefaultGd=NULL;

//	コンストラクタ
CMgl3DManager::CMgl3DManager()
{
	_MGL_DEBUGLOG( "CMgl3DManager のインスタンスが生成されました。" );
	CMglStackInstance("CMgl3DManager::CMgl3DManager");
...
CMgl3DManager::~CMgl3DManager()
{
	_MGL_DEBUGLOG( "+ CMgl3DManager::デストラクタ" );
	CMglStackInstance("CMgl3DManager::~CMgl3DManager");
	Release();
	_MGL_DEBUGLOG( "- CMgl3DManager::デストラクタ" );
	g_2dgCount--;
...
{
	_MGL_DEBUGLOG( "+ CMgl3DManager::Release()" );
	CMglStackInstance("CMgl3DManager::Release");
...
//	初期化及び開始
void CMgl3DManager::Init( HWND hWnd, int nDispX, int nDispY, BOOL bFullscreen,
							  D3DFORMAT formatTexture, DWORD dwD3dDeviceMode, UINT nAdapterNo )
{
	_MGL_DEBUGLOG( "+ CMgl3DManager::Init()" );
	CMglStackInstance("CMgl3DManager::Init");
	...
	*/
	...
	if ( m_pD3d->GetDeviceCaps( nAdapterNo, D3DDEVTYPE_HAL, &caps ) != D3D_OK )
	{
		...
		MyuAssert( m_pD3d->GetDeviceCaps( nAdapterNo, D3DDEVTYPE_REF, &caps ), D3D_OK,
			"CMgl3DManager::Init()  HAL/REFデバイス情報の取得に失敗。" );
	...
	MyuAssert( this->m_pD3d->GetAdapterDisplayMode( nAdapterNo, &dispMode ), D3D_OK,
		"CMgl3DManager::Init()  ディスプレイモードの取得に失敗。" );
	...
	if ( m_bUseSprite )
		MyuAssert( D3DXCreateSprite( m_pD3dDev, &m_pSprite ), D3D_OK,
			"CMgl3DManager::Init()  D3DXCreateSprite()に失敗" );
	*/
	InitSprite();

	_MGL_DEBUGLOG( "- CMgl3DManager::Init()" );
}
void CMgl3DManager::InitSprite()
{
	if ( m_pD3dDev == NULL )
		MyuThrow( 0, "CMgl3DManager::InitSprite()  m_pD3dDevがNULL" );
...
void CMgl3DManager::Direct3DCreate()
{
	CMglStackInstance("CMgl3DManager::Direct3DCreate");

	if ( m_pD3d != NULL )
		//MyuThrow2( 0, 0x0201, "CMgl3DManager::Direct3DCreate()  D3Dオブジェクトは既あります。" );
	...
	if ( NULL == m_pD3d )
		MyuThrow2( 0, MGLMSGNO_GRP_MANAGER_D3DCREATE_FAILED,
			"CMgl3DManager::Direct3DCreate()  Direct3DCreate8()に失敗。" );
}

//	初期化及び開始のEx版
void CMgl3DManager::InitEx( D3DPRESENT_PARAMETERS* pPresentParam, DWORD dwD3dDeviceMode, D3DFORMAT formatTexture, int nAdapterNo, HWND hFocusWindow )
{
	_MGL_DEBUGLOG( "+ CMgl3DManager::InitEx()" );
	CMglStackInstance("CMgl3DManager::InitEx");
...
*/
	...
	MyuAssert( this->m_pD3dDev->SetVertexShader( FVF_MYU_VERTEX ), D3D_OK,
		"CMgl3DManager::Init()  SetVertexShader()に失敗" );

	//	バックバッファの情報を取得
	MyuAssert( GetBackBuffer()->GetDesc( &backBufferDesc ), D3D_OK,
		"CMgl3DManager::Init()  GetBackBuffer()->GetDesc()に失敗" );
...
BOOL CMgl3DManager::D3dCreateDevice( DWORD dwD3dDeviceFlg, D3DPRESENT_PARAMETERS* pPresentParam, int nAdapterNo, HWND hFocusWindow )
{
	//_MGL_DEBUGLOG( "+ CMgl3DManager::D3dCreateDevice()" );
	CMglStackInstance("CMgl3DManager::D3dCreateDevice");
	...
	case D3D_DEVICE_FLG_REF:
		...
		_MGL_DEBUGLOG( "CMgl3DManager::D3dCreateDevice()  REFにて試行します。" );
		break;
	...
	case D3D_DEVICE_FLG_HAL:
		...
		vertexFlag = D3DCREATE_SOFTWARE_VERTEXPROCESSING;
		_MGL_DEBUGLOG( "CMgl3DManager::D3dCreateDevice()  HALにて試行します。" );
		break;
	...
	case D3D_DEVICE_FLG_T_AND_L_HAL:
		...
		_MGL_DEBUGLOG( "CMgl3DManager::D3dCreateDevice()  T&L HALにて試行します。" );
		break;
	...
	case D3D_DEVICE_FLG_HW_SW_MIX:
		...
		_MGL_DEBUGLOG( "CMgl3DManager::D3dCreateDevice()  ソフト＆ハードの混合にて試行します。" );
		break;

	//	不明な定数
	default:
		MyuThrow( 0, "CMgl3DManager::D3dCreateDevice() 不明なデバイスフラグです。(dwD3dDeviceFlg=0x%08X)", dwD3dDeviceFlg );
	}
    ...
    {
		_MGL_DEBUGLOG( "CMgl3DManager::D3dCreateDevice()  失敗。" );
		return FALSE;
	}
	else
	{
		_MGL_DEBUGLOG( "CMgl3DManager::D3dCreateDevice()  成功。" );
		return TRUE;
	}
}

//	フルスクリーン時のディスプレイモードが可能かどうかチェックする
void CMgl3DManager::IsValidDisplayMode( int nWidth, int nHeight, D3DFORMAT Format, int nRefreshRate )
{
...
void CMgl3DManager::Clear( D3DCOLOR color )
{
	CMglStackInstance("CMgl3DManager::Clear");
   //m_pD3dDev->Clear( 0, NULL, D3DCLEAR_TARGET, color, 1.0f, 0 );// ZBufferもクリアする
	Paint( NULL, color );
}

void CMgl3DManager::Paint( RECT* rect, D3DCOLOR color )
{
	CMglStackInstance("CMgl3DManager::Paint");
	if ( m_pD3dDev == NULL )
		MyuThrow( 0, "CMgl3DManager::Paint()  m_pD3dDevがNULL" );
...
void CMgl3DManager::SetAlphaBlendSetting( DWORD alphaOp )
{
...
//	アルファブレンドを有効にする
void CMgl3DManager::EnabledAlphaBlend() {
	m_pD3dDev->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);
}

//	アルファブレンドを無効にする
void CMgl3DManager::DisableAlphaBlend() {
	m_pD3dDev->SetRenderState(D3DRS_ALPHABLENDENABLE, FALSE);
}
*/

//	アルファ値をどう処理するか設定する
void CMgl3DManager::SetAlphaMode( DWORD dwAlphaOption )
{
	if ( m_pD3dDev == NULL )
		MyuThrow( 0, "CMgl3DManager::SetAlphaMode()  m_pD3dDevがNULL" );
	...
	{
		MyuThrow( 0, "CMgl3DManager::SetAlphaMode()  アルファオプション %u は不明なオプションです。", dwAlphaOption );
	}
...
//	画面への反映を行う
void CMgl3DManager::UpdateScreen()
{
	if ( m_pD3dDev == NULL )
		MyuThrow( 0, "CMgl3DManager::UpdateScreen()  m_pD3dDevがNULL" );
	...
	//	シーンの開始
	MyuAssert( m_pD3dDev->BeginScene(), D3D_OK,
		"CMgl3DManager::BeginScene()  シーンの開始に失敗。" );
}

/*
//	シーンの開始
void CMgl3DManager::BeginScene()
{
	MyuAssert( m_pD3dDev->BeginScene(), D3D_OK,
		"CMgl3DManager::BeginScene()  シーンの開始に失敗。" );
}
...
//	シーンの終了
void CMgl3DManager::EndScene()
{
	m_pD3dDev->EndScene();
}

//	画面への反映
void CMgl3DManager::Present()
{
	m_pD3dDev->Present( NULL, NULL, NULL, NULL );
}
*/
...
//	自動開放用
void CMgl3DManager::AddRefOfAutoRelease( CMyuReleaseBase* pRef )
{
	//	既に無いかチェック
...
void CMgl3DManager::RmRefOfAutoRelease( CMyuReleaseBase* pRef )
{
	//	存在するかチェック
	...
	if ( itr == m_autoReleaseAry.end() )
	{
		/*
		char szWork[256];
		sprintf( szWork, "CMgl3DManager::RmRefOfAutoRelease()  0x%08x のリファレンスは存在しません。", pRef );
		MessageBox( NULL, szWork, NULL, NULL );
		*/
		return;
		//MyuThrow( 0, "CMgl3DManager::RmRefOfAutoRelease()  0x%08x のリファレンスは存在しません。", pRef );
	}
...
//	スプライト描画
void CMgl3DManager::SpriteDraw( CMglTexture *pTexture, float x, float y, CONST RECT* pSrcRect, D3DCOLOR color,
		float fScaleX, float fScaleY, float fRotationCenterX, float fRotationCenterY, float fAngle )
{
	...
	*/
	...
	MyuAssert( m_pSprite->Draw( pTexture->GetDirect3dTexturePtr(),
		...
		"CMgl3DManager::SpriteDraw()  m_pSprite->Draw()に失敗" );
}

//	スプライトのBegin()
void CMgl3DManager::SpriteBegin()
{
	if( m_pSprite != NULL && m_bSpriteBegun == FALSE ){
		MyuAssert( m_pSprite->Begin(), D3D_OK,
			"CMgl3DManager::SpriteBegin()  m_pSprite->Begin()に失敗" );
		m_bSpriteBegun = TRUE;
	}
}
//	スプライトのEnd()
void CMgl3DManager::SpriteEnd()
{
	if( m_pSprite != NULL && m_bSpriteBegun == TRUE ){
		MyuAssert( m_pSprite->End(), D3D_OK,
			"CMgl3DManager::SpriteEnd()  m_pSprite->End()に失敗" );
		m_bSpriteBegun = FALSE;
	}
}
...
////////////////////////////////////////////////////////////
//
//	デフォルト CMgl3DManager インスタンス関連
//
CMgl3DManager* GetDefaultGd()
{
	CMglStackInstance("CMgl3DManager::GetDefaultGd");
	if ( g_pDefaultGd == NULL )
	{
		MyuThrow2( 0, MGLMSGNO_GRP_MANAGER_MULTI_INSTANCE,
			"CMgl3DManager のインスタンスが存在しないか複数存在するため省略する事は出来ません。" );
	}
	return g_pDefaultGd;
}
