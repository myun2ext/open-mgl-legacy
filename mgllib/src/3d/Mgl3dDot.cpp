...
//CMglTexture* CMgl3dDot::m_pLineTex;
...
//	描画
void CMgl3dDot::Draw(float fStartX, float fStartY, float fEndX, float fEndY, D3DCOLOR startColor, D3DCOLOR endColor, float width)
{
	InitCheck();
                                                  ...
                                                  D3DPOOL_DEFAULT, &pVertexBuffer ) ) )
    {
        MyuThrow( MGLMSGNO_DRAW_LINE_DRAW_FAILED,
			"CMgl3dDot::Draw()  m_d3d->CreateVertexBuffer()に失敗" );
    }
    ...
    if( FAILED( pVertexBuffer->Lock( 0, MGL_VERTEXES_SIZE, &pVertices, 0 ) ) )
        MyuThrow( MGLMSGNO_DRAW_LINE_DRAW_FAILED,
			"CMgl3dDot::Draw()  pVertexBuffer->Lock()に失敗" );

	if ( pVertices == NULL )
        MyuThrow( MGLMSGNO_DRAW_LINE_DRAW_FAILED,
			"CMgl3dDot::Draw()  pVertices is NULL." );
	...
	MyuAssert( m_d3d->DrawPrimitiveUP( D3DPT_TRIANGLESTRIP, 1, vertices, sizeof(MYU_VERTEX) ), D3D_OK,
		"CMgl3dDot::Draw()  DrawPrimitiveUP()に失敗" );
	*/
...
void CMgl3dDot::Draw2(float fStartX, float fStartY, float fEndX, float fEndY, D3DCOLOR startColor, D3DCOLOR endColor, float width)
{
	InitCheck();
