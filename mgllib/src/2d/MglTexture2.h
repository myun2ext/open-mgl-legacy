...
protected:
	...
	void CreateCheck() {
		...
		//if ( createFlg == FALSE ){
		if ( m_pTexture == NULL )
			MyuThrow( 0, "CMglTexture2 未作成であるにも関わらず操作を行おうとしました。Create()系メソッドを呼んでください。" );
	}
	...
	void LockedCheck(){
		if ( m_bLocked == TRUE )
			MyuThrow(0, "CMglTexture2 ロックされています。ロックを開放してください。\r\n\r\n"
				"考えられる可能性 :\r\n"
...
private:
	...
	void _GetSurface(){
		MyuAssert( m_pTexture->GetSurfaceLevel(0, &m_pSurface), D3D_OK,
			"CMglTexture2::GetSurface()  GetSurfaceLevel()に失敗" );
	}
public:
	//	コンストラクタ・デストラクタ
	CMglTexture2();
...
typedef CMglTexture2 CMglSquareTexture;
