...
//void CMglTexture2::CreateTextureFromFile( LPCSTR szFileName, BOOL bRenderTarget, D3DCOLOR colorKey )
void CMglTexture2::CreateTextureFromFileEx( LPCSTR szFileName, int nForceBmpWidth, int nForceBmpHeight,
...
//	生成
void CMglTexture2::Create( int x, int y, BOOL bRenderTarget )
{
	_MGL_DEBUGLOG( "+ CMglTexture2::Create()" );
	...
	MyuAssert( m_pTexture->GetSurfaceLevel(0, &m_pSurface), D3D_OK,
		"CMglTexture2::Create()  GetSurfaceLevel()に失敗" );

	//	スプライト作成
	MyuAssert( D3DXCreateSprite( d3d, &this->m_pSprite ), D3D_OK,
		"CMglTexture2::Init  D3DXCreateSprite()に失敗" );
...
//	新・描画方式
void CMglTexture2::Draw(float x, float y, D3DCOLOR color )
{
	//	頂点初期情報取得
...
//	描画（二次元固定）
void CMglTexture2::Draw( MGL_SQUARE_VERTEXS *pMglSqVertexs, BOOL bVertexRevise )
{
	Draw( (MYU_VERTEX*)pMglSqVertexs, 2, bVertexRevise );
}

//	描画（頂点数不定）
void CMglTexture2::Draw( MYU_VERTEX *pMyuVertex, UINT nPrimitiveCount, BOOL bVertexRevise )
{
...
//	DrawPrimitiveUP
void CMglTexture2::DrawPrimitiveUP( D3DPRIMITIVETYPE primitiveType, UINT nPrimitiveCount,
...
void CMglTexture2::SetD3dStageTexture(DWORD nStage)
{
	//	テクスチャの設定
	MyuAssert( d3d->SetTexture(nStage, m_pTexture), D3D_OK,
		"CMglTexture2::SetD3dTexture()  d3d->SetTexture()に失敗" );
}

///////////////////////////////////////////////////////////////////////////

//	BMPの頂点情報取得
void CMglTexture2::GetBmpVertexs( MGL_SQUARE_VERTEXS *pMglSqVertexs )
{
...
#endif
		"CMglTexture2::SetRender()  SetRenderTarget()に失敗" );
}
...
//	クリアする
void CMglTexture2::Clear( D3DCOLOR color )
{
	Paint(NULL,color);
}

void CMglTexture2::Clear__( D3DCOLOR color )
{
	CreateCheck();	//	Createチェック
	LockedCheck();
...
#endif
			"CMglTexture2::Clear()  レンダーを戻すのに失敗" );
	}
	else
	{
		/*
		//	別にクリアされたサーフェスを作成してそこからコピー、と言う面倒な処理(´Д`)
		CMglTexture2 workSurface;
...
//	指定された矩形領域を塗りつぶす
void CMglTexture2::Paint( RECT* pRect, D3DCOLOR color )
{
	CreateCheck();	//	Createチェック
	LockedCheck();
...
/*
CMglTexture2::iterator CMglTexture2::begin()
{

}

CMglTexture2::iterator CMglTexture2::end()
{
