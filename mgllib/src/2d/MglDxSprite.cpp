...
int g_nSfcCount=0;
int g_2dgCount=0;
CMglDxSprite *g_pDefaultGd=NULL;

//	コンストラクタ
CMglDxSprite::CMglDxSprite()
{
	_MGL_DEBUGLOG( "CMglDxSprite のインスタンスが生成されました。" );
	CMglStackInstance("CMglDxSprite::CMglDxSprite");
...
CMglDxSprite::~CMglDxSprite()
{
	_MGL_DEBUGLOG( "+ CMglDxSprite::デストラクタ" );
	CMglStackInstance("CMglDxSprite::~CMglDxSprite");
	Release();
	_MGL_DEBUGLOG( "- CMglDxSprite::デストラクタ" );
	g_2dgCount--;
...
//	開放
void CMglDxSprite::Release()
{
	_MGL_DEBUGLOG( "+ CMglDxSprite::Release()" );
	CMglStackInstance("CMglDxSprite::Release");
...
//	初期化及び開始
void CMglDxSprite::Init( HWND hWnd, int nDispX, int nDispY, BOOL bFullscreen, BOOL bUse3d,
							  D3DFORMAT formatTexture, DWORD dwD3dDeviceMode, UINT nAdapterNo )
{
	_MGL_DEBUGLOG( "+ CMglDxSprite::Init()" );
	CMglStackInstance("CMglDxSprite::Init");
	...
	*/
	...
	if ( m_pD3d->GetDeviceCaps( nAdapterNo, D3DDEVTYPE_HAL, &caps ) != D3D_OK )
	{
		...
		MyuAssert( m_pD3d->GetDeviceCaps( nAdapterNo, D3DDEVTYPE_REF, &caps ), D3D_OK,
			"CMglDxSprite::Init()  HAL/REFデバイス情報の取得に失敗。" );
	...
	MyuAssert( this->m_pD3d->GetAdapterDisplayMode( nAdapterNo, &dispMode ), D3D_OK,
		"CMglDxSprite::Init()  ディスプレイモードの取得に失敗。" );
...
*/
	...
	if ( m_bUseSprite )
		MyuAssert( D3DXCreateSprite( m_pD3dDev, &m_pSprite ), D3D_OK,
			"CMglDxSprite::Init()  D3DXCreateSprite()に失敗" );
	*/
	InitSprite();
...
void CMglDxSprite::Direct3DCreate()
{
	CMglStackInstance("CMglDxSprite::Direct3DCreate");

	if ( m_pD3d != NULL )
		//MyuThrow2( 0, 0x0201, "CMglDxSprite::Direct3DCreate()  D3Dオブジェクトは既あります。" );
	...
	if ( NULL == m_pD3d )
		MyuThrow2( MGLMSGNO_GRPMGR(40), MGLMSGNO_GRP_MANAGER_D3DCREATE_FAILED,
			"CMglDxSprite::Direct3DCreate()  Direct3DCreate8()に失敗。" );
}

//	初期化及び開始のEx版
void CMglDxSprite::InitEx( D3DPRESENT_PARAMETERS* pPresentParam, DWORD dwD3dDeviceMode, D3DFORMAT formatTexture, int nAdapterNo, HWND hFocusWindow )
{
	_MGL_DEBUGLOG( "+ CMglDxSprite::InitEx()" );
	CMglStackInstance("CMglDxSprite::InitEx");
	...
	MyuAssert2( GetBackBuffer()->GetDesc( &backBufferDesc ), D3D_OK,
		MGLMSGNO_GRPMGR(44), "CMglDxSprite::Init()  GetBackBuffer()->GetDesc()に失敗" );
	...
	MyuAssert2( m_pD3dDev->BeginScene(), D3D_OK,
		MGLMSGNO_GRPMGR(46), "CMglDxSprite::Init()  シーンの開始に失敗。" );
...
//	Vertexを設定
void CMglDxSprite::SetFVF(DWORD dwFvf)
{
#if _MGL_DXVER == 8
	MyuAssert2( this->m_pD3dDev->SetVertexShader( dwFvf ), D3D_OK,
		MGLMSGNO_GRPMGR(93), "CMglDxSprite::SetFVF()  m_pD3dDev->SetVertexShader()に失敗" );
#else
	MyuAssert2( this->m_pD3dDev->SetFVF( dwFvf ), D3D_OK,
		MGLMSGNO_GRPMGR(93), "CMglDxSprite::SetFVF()  m_pD3dDev->SetFVF()に失敗" );
#endif
}

//	Direct3D の CreateDevice()を独自フラグにより呼び出す
BOOL CMglDxSprite::D3dCreateDevice( DWORD dwD3dDeviceFlg, D3DPRESENT_PARAMETERS* pPresentParam, int nAdapterNo, HWND hFocusWindow )
{
	//_MGL_DEBUGLOG( "+ CMglDxSprite::D3dCreateDevice()" );
	CMglStackInstance("CMglDxSprite::D3dCreateDevice");
	...
	case D3D_DEVICE_FLG_REF:
		...
		_MGL_DEBUGLOG( "CMglDxSprite::D3dCreateDevice()  REFにて試行します。" );
		break;
	...
	case D3D_DEVICE_FLG_HAL:
		...
		vertexFlag = D3DCREATE_SOFTWARE_VERTEXPROCESSING;
		_MGL_DEBUGLOG( "CMglDxSprite::D3dCreateDevice()  HALにて試行します。" );
		break;
	...
	case D3D_DEVICE_FLG_T_AND_L_HAL:
		...
		_MGL_DEBUGLOG( "CMglDxSprite::D3dCreateDevice()  T&L HALにて試行します。" );
		break;
	...
	case D3D_DEVICE_FLG_HW_SW_MIX:
		...
		_MGL_DEBUGLOG( "CMglDxSprite::D3dCreateDevice()  ソフト＆ハードの混合にて試行します。" );
		break;

	//	不明な定数
	default:
		MyuThrow( MGLMSGNO_GRPMGR(62), "CMglDxSprite::D3dCreateDevice() 不明なデバイスフラグです。(dwD3dDeviceFlg=0x%08X)", dwD3dDeviceFlg );
	}
    ...
    {
		_MGL_DEBUGLOG( "CMglDxSprite::D3dCreateDevice()  失敗。" );
		return FALSE;
	}
	else
	{
		_MGL_DEBUGLOG( "CMglDxSprite::D3dCreateDevice()  成功。" );
		return TRUE;
	}
}

//	フルスクリーン時のディスプレイモードが可能かどうかチェックする
void CMglDxSprite::IsValidDisplayMode( int nWidth, int nHeight, D3DFORMAT Format, int nRefreshRate )
{
...
void CMglDxSprite::Clear( D3DCOLOR color )
{
	CMglStackInstance("CMglDxSprite::Clear");
   //m_pD3dDev->Clear( 0, NULL, D3DCLEAR_TARGET, color, 1.0f, 0 );// ZBufferもクリアする
	...
	if ( m_bUse3d )
		MyuAssert2( m_pD3dDev->Clear( 0, NULL, D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(0,0,0), 1.0f, 0 ), D3D_OK,
			MGLMSGNO_GRPMGR(111), "CMglDxSprite::Clear()  m_pD3dDev->Clear()に失敗" );
}

void CMglDxSprite::Paint( RECT* rect, D3DCOLOR color )
{
	CMglStackInstance("CMglDxSprite::Paint");
	if ( m_pD3dDev == NULL )
		MyuThrow( 0, "CMglDxSprite::Paint()  m_pD3dDevがNULL" );
    ...
    MyuAssert2( m_pD3dDev->Clear( rectCount, (D3DRECT*)rect, D3DCLEAR_TARGET, color, 1.0f, 0 ), D3D_OK,
		MGLMSGNO_GRPMGR(110), "CMglDxSprite::Clear()  m_pD3dDev->Clear()に失敗" );
}
...
void CMglDxSprite::SetAlphaBlendSetting( DWORD alphaOp )
{
...
//	アルファブレンドを有効にする
void CMglDxSprite::EnabledAlphaBlend() {
	m_pD3dDev->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);
}

//	アルファブレンドを無効にする
void CMglDxSprite::DisableAlphaBlend() {
	m_pD3dDev->SetRenderState(D3DRS_ALPHABLENDENABLE, FALSE);
}
*/

//	アルファ値をどう処理するか設定する
void CMglDxSprite::SetAlphaMode( DWORD dwAlphaOption )
{
	if ( m_pD3dDev == NULL )
		MyuThrow( MGLMSGNO_GRPMGR(69), "CMglDxSprite::SetAlphaMode()  m_pD3dDevがNULL" );
	...
	{
		MyuThrow( 0, "CMglDxSprite::SetAlphaMode()  アルファオプション %u は不明なオプションです。", dwAlphaOption );
	}
...
//	画面への反映を行う
void CMglDxSprite::UpdateScreen()
{
	if ( m_pD3dDev == NULL )
		MyuThrow( MGLMSGNO_GRPMGR(30), "CMglDxSprite::UpdateScreen()  m_pD3dDevがNULL" );
	...
	//	シーンの開始
	MyuAssert2( m_pD3dDev->BeginScene(), D3D_OK,
		MGLMSGNO_GRPMGR(31), "CMglDxSprite::BeginScene()  シーンの開始に失敗。" );
}

/*
//	シーンの開始
void CMglDxSprite::BeginScene()
{
	MyuAssert( m_pD3dDev->BeginScene(), D3D_OK,
		"CMglDxSprite::BeginScene()  シーンの開始に失敗。" );
}

//	シーンの終了
void CMglDxSprite::EndScene()
{
	m_pD3dDev->EndScene();
}

//	画面への反映
void CMglDxSprite::Present()
{
	m_pD3dDev->Present( NULL, NULL, NULL, NULL );
}
*/
...
//	自動開放用
void CMglDxSprite::AddRefOfAutoRelease( CMyuReleaseBase* pRef )
{
	//	既に無いかチェック
...
void CMglDxSprite::RmRefOfAutoRelease( CMyuReleaseBase* pRef )
{
	//	存在するかチェック
	...
	if ( itr == m_autoReleaseAry.end() )
	{
		/*
		char szWork[256];
		sprintf( szWork, "CMglDxSprite::RmRefOfAutoRelease()  0x%08x のリファレンスは存在しません。", pRef );
		MessageBox( NULL, szWork, NULL, NULL );
		*/
		return;
		//MyuThrow( 0, "CMglDxSprite::RmRefOfAutoRelease()  0x%08x のリファレンスは存在しません。", pRef );
	}
...
//	スプライト描画
void CMglDxSprite::SpriteDraw( CMglTexture *pTexture, float x, float y, CONST RECT* pSrcRect, D3DCOLOR color,
...
void CMglDxSprite::DrawPrimitiveUpMyuVertex(MGL_VERTEX *pMglVertexs, int nVertexCount, D3DPRIMITIVETYPE primitiveType)
{
	MyuAssert2( m_pD3dDev->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, nVertexCount, pMglVertexs, sizeof(MGL_VERTEX)), D3D_OK,
			MGLMSGNO_GRPMGR(130), "CMglDxSprite::DrawPrimitiveUpMyuVertexSquare()  DrawPrimitiveUP()に失敗" );
}

//	Zバッファのフォーマット対応状況をチェック
BOOL CMglDxSprite::CheckDepthStencilFormat( UINT nAdapterNo,
...
D3DFORMAT CMglDxSprite::CheckAutoDepthStencilFormat( UINT nAdapterNo,
...
//
CMglDxSprite* GetDefaultGd()
{
	CMglStackInstance("CMglDxSprite::GetDefaultGd");
	if ( g_pDefaultGd == NULL )
	{
		MyuThrow2( MGLMSGNO_GRPMGR(196), MGLMSGNO_GRP_MANAGER_MULTI_INSTANCE,
			"CMglDxSprite のインスタンスが存在しないか複数存在するため省略する事は出来ません。" );
	}
	return g_pDefaultGd;
}
