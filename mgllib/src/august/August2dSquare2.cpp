...
//	フォントの再構築
void CAugust2dSquare2::ReCreateFont()
{
	m_pGrp = (CMglGraphicManager*)MyuAssertNull(GetValPtr(AUGUST_VALKEY_GRP),
		"CAugust2dSquare2: GetValPtr(AUGUST_VALKEY_GRP) の取得に失敗。");
...
//	オプションのDWORD取得
DWORD CAugust2dSquare2::GetDrawInternalOption()
{
	DWORD dwOption = 0;
...
//	オプション設定
void CAugust2dSquare2::SetOption(agh::AGHDWORD dwOption)
{
	_BASE::SetOption(dwOption);
	OnChanged();
}
void CAugust2dSquare2::SetFontName(const char* szFontName){
	_BASE::SetFontName(szFontName);
	OnChanged();
}
void CAugust2dSquare2::SetFontPoint(int point){
	_BASE::SetFontPoint(point);
	OnChanged();
}
void CAugust2dSquare2::SetColor(agh::COLOR color){
